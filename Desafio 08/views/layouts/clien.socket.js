...
const renderMessage = (messagesData) => {
    const html = messagesData.map((messageInfo) => {
        return `<div> <strong style="color: green">${messageInfo.username}</strong><span style="color: brown">[${messageInfo.time}]:</span> <em style="color: white; font-style: italic">${messageInfo.message}</em> </div>`;
    });
